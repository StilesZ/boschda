...
if (!empty($_REQUEST['id'])) {
    ...
    $sql = "update weld_type set name='{$name}' where id={$id}";
    ...
    $check=check($name,$mysqli,$id);
    $sql = "insert into weld_type(name) values ('{$name}')";
    $result = $mysqli->query($sql);
    if ($result) {
...
function check($name,$mysqli,$id){
    $check=$mysqli->query("select name from weld_type where name='{$name}' and id!='{$id}'");
    if($check->num_rows>0){
