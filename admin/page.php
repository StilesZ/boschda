...
class page
{
    ...
    function fpage()
    {
        ...
        $fpage = '<div style="font:12px \'\5B8B\4F53\',san-serif;text-align: center;">';
        if (count($arr) < 1)
    ...
    private function firstprev()
    {
        if ($this->page > 1) {
            ...
            $controllerName = strtolower($urlArray[0]);
            $actionName = strtolower($urlArray[1]);
            $pageName = strtolower($urlArray[2]);
            $ul=$actionName."/".$pageName;

//            $str = "&nbsp;<a href='{$this->uri}page=1'>{$this->config["first"]}</a>&nbsp;";
//            $str .= "<a href='{$this->uri}page=" . ($this->page - 1) . "'>{$this->config["prev"]}</a>&nbsp;";

            $str = "&nbsp;<a href='#' onclick=action('{$ul}page=1')>{$this->config["first"]}</a>&nbsp;";
            $str .= "<a href='#' onclick=action('{$ul}page=" .($this->page - 1)."')>{$this->config["prev"]}</a>&nbsp;";
            return $str;
        }
    ...
    private function pageList()
    {
        ...
        for ($i = $inum; $i >= 1; $i--) {
            ...
            if ($page >= 1)
                $linkPage .= "<a href='#' onclick=action('{$ul}page=$page')>{$page}</a>&nbsp;";
        }
        /*当前页的信息 */
        if ($this->pageNum > 1)
...
//                $linkPage .= "<a href='{$this->uri}page={$page}'>{$page}</a>&nbsp;";
    ...
    private function nextlast()
    {
        ...
        if ($this->page != $this->pageNum) {
            $str = "&nbsp;<a href='#' onclick=action('{$ul}page=" . ($this->page + 1) . "')>{$this->config["next"]}</a>&nbsp;";
            $str .= "&nbsp;<a href='#' onclick=action('{$ul}page=" . ($this->pageNum) . "')>{$this->config["last"]}</a>&nbsp;";
            return $str;
        }
    }
    ...
    private function goPage()
    {
        ...
        if ($this->pageNum > 1) {
            return '<input style="width:20px;height:17px !important;height:18px;border:1px solid #CCCCCC;" type="text" onkeydown="javascript:if(event.keyCode==13){var page=(this.value>' . $this->pageNum . ')?' . $this->pageNum . ':this.value;action(\'' . $ul . 'page=\'+page+\'\')}" value="' . $this->page . '"><input style="cursor:pointer;width:25px;height:18px;border:1px solid #CCCCCC;" type="button" value="GO" onclick="javascript:var page=(this.previousSibling.value>' . $this->pageNum . ')?' . $this->pageNum . ':this.previousSibling.value;action(\'' . $ul . 'page=\'+page+\'\')">&nbsp;';
        }
    }
